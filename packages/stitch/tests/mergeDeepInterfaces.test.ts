...
const localResolvers = {
    ...
    Query: {
        ...
        categoryDecoratorsByIds: (root, {ids}) => {
            console.log('ex')
            return ids.map(id => decorators.find(m => m.txId === id && m.decorationType === 'CategoryDecorator') || null);
        },
        documentDecoratorsByIds: (root, {ids}) => {
            console.log('ex')
            return ids.map(id => decorators.find(m => m.txId === id && m.decorationType === 'DocumentDecorator') || null);
        },
    }
}
...
const remoteSchema = makeExecutableSchema({typeDefs: remoteSchemaGQL, resolvers: remoteResolvers})
